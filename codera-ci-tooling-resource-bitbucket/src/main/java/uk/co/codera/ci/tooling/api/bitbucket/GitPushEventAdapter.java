...
public class GitPushEventAdapter {
	...
	public GitPushEventAdapter(String bitBucketServerName,
			int bitBucketServerPort) {
	...
	public GitPushEvent from(PushEvent event) {
		...
		return GitPushEvent.aGitPushEvent().pushType(refChange.getType())
				.reference(GitReference.from(refChange.getRefId()))
	...
	private String repositoryUrl(PushEvent event) {
		...
		return String.format(TEMPLATE_REPOSITORY_URL, this.bitBucketServerName,
				this.bitBucketServerPort, repository.getProject().getKey()
						.toLowerCase(), repository.getSlug());
	}
}