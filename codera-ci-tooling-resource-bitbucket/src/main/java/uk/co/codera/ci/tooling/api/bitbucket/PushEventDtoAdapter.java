...
public class PushEventDtoAdapter {
    ...
    public GitPushEvent from(PushEventDto event) {
        ...
        return GitPushEvent.aGitPushEvent().pushType(refChange.getType()).reference(GitReference.from(refChange.getRefId())).repositoryName(event.getRepository().getSlug())
    ...
    private String repositoryUrl(PushEventDto event) {
        ...
        return String.format(TEMPLATE_REPOSITORY_URL, this.bitBucketServerName, this.bitBucketServerPort, repository.getProject().getKey().toLowerCase(), repository.getSlug());
    }
}