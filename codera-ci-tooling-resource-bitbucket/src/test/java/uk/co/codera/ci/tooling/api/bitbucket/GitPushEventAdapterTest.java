...
public class GitPushEventAdapterTest {
	...
	@Before
	public void before() {
		this.adapter = new GitPushEventAdapter(BITBUCKET_SERVER_NAME,
	...
	public void shouldMapGitPushTypeFromPushEvent() {
		assertThat(from(aValidAddRefChange()).getPushType(),
				is(GitPushType.ADD));
	}

	@Test
	public void shouldMapRepositoryNameFromPushEvent() {
		assertThat(from(aValidRepository().slug("jeff")).getRepositoryName(),
				is("jeff"));
	}

	@Test
	public void shouldConstructGitReferenceFromPushEvent() {
		assertThat(from(aValidAddRefChange().refId("refs/heads/master"))
				.getReference(),
	...
	public void shouldConstructRepositoryUrlFromBitBucketServerAndPushEventInformation() {
		assertThat(
				from(
						aValidRepository().slug("repo").with(
								aValidProject().key("proj").build()))
						.getRepositoryUrl(),
	...
	public void shouldMakeProjectKeyLowercaseInRepositoryUrl() {
		assertThat(
				from(
						aValidRepository().with(
								aValidProject().key("PROJ").build()))
						.getRepositoryUrl(), containsString("/proj/"));
	}
