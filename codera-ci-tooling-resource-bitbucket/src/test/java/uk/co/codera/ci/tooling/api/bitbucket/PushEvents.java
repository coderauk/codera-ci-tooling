...
public class PushEvents {
	...
	public static PushEvent.Builder aValidPushEvent() {
		return PushEvent.aPushEvent().with(aValidRepository().build())
				.with(aValidAddRefChange().build());
	}

	public static Repository.Builder aValidRepository() {
		return Repository.aRepository().slug(REPOSITORY_SLUG)
				.with(aValidProject().build());
	}
	...
	public static RefChange.Builder aValidAddRefChange() {
		return RefChange.aRefChange().refId(GIT_REFERENCE_ID)
				.type(GitPushType.ADD);
	}
}