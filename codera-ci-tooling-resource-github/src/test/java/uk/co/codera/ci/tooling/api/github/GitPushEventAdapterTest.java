...
public class GitPushEventAdapterTest {
	...
	public void shouldMapDeleteGitPushTypeFromEventType() {
		assertThat(from(EVENT_TYPE_DELETE).getPushType(),
				is(GitPushType.DELETE));
	}

	@Test
	public void shouldMapRepositoryNameFromPushEvent() {
		assertThat(from(aValidRepository().name("boatymcboatface"))
				.getRepositoryName(), is("boatymcboatface"));
	}

	@Test
	public void shouldMapRepositorySshUrlFromPushEvent() {
		assertThat(from(aValidRepository().sshUrl("ssh:mcSsh:/repo"))
				.getRepositoryUrl(), is("ssh:mcSsh:/repo"));
	}
