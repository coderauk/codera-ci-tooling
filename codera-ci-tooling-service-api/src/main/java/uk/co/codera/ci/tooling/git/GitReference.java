...
public class GitReference {
	...
	private static final Pattern PATTERN_VALID_GIT_REFERENCE = Pattern
			.compile("refs/\\w*/(.*)");
	...
	public String shortBranchName() {
		...
		if (longBranchName.contains(BRANCH_DELIMITER)) {
			return longBranchName.substring(longBranchName
					.indexOf(BRANCH_DELIMITER) + 1);
		}
	...
	private Matcher validate(String reference) {
		...
		if (!matcher.matches()) {
			throw new IllegalArgumentException(String.format(
					"Invalid git reference [%s]", reference));
		}
		return matcher;
	}
