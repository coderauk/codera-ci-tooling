...
public class GitReferenceTest {
	...
	public void toStringShouldRepresentFullReference() {
		assertThat(GitReference.from(VALID_REFERENCE).toString(),
				is(equalTo(VALID_REFERENCE)));
	}

	@Test
	public void shouldReportBranchNameCorrectly() {
		assertThat(GitReference.from(VALID_REFERENCE).branchName(),
				is("feature/JT-001-first-branch"));
	}

	@Test
	public void shouldNotShortenBranchNameForMaster() {
		assertThat(GitReference.from("refs/heads/master").shortBranchName(),
				is("master"));
	}

	@Test
	public void shouldNotShortenBranchNameForCustomBranchName() {
		assertThat(GitReference.from("refs/heads/custom-branch-name")
				.shortBranchName(), is("custom-branch-name"));
	}

	@Test
	public void shouldShortenBranchNameForFeatureBranchName() {
		assertThat(GitReference.from("refs/heads/feature/some-feature")
				.shortBranchName(), is("some-feature"));
	}

	@Test
	public void invalidReferenceShouldThrowException() {
		this.expectedException.expect(IllegalArgumentException.class);
		this.expectedException.expectMessage("Invalid git reference ["
				+ INVALID_REFERENCE + "]");
	...
	public void differentInstanceButSameReferenceShouldBeEqual() {
		assertThat(GitReference.from("refs/heads/master"),
	...
	public void differentInstanceButSameReferenceShouldHaveSameHashCode() {
		assertThat(GitReference.from("refs/heads/master").hashCode(),
				is(equalTo(GitReference.from("refs/heads/master").hashCode())));
	}
