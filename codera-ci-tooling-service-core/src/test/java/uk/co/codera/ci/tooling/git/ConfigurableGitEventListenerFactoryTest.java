...
public class ConfigurableGitEventListenerFactoryTest {
	...
	public void shouldHaveDefaultListenerForAnyPushType() {
		assertThat(newFactory().listenerFor(GitPushType.ADD),
				is(notNullValue()));
	}
	...
	public void defaultListenerIfNotSpecifiedShouldLogEvent() {
		assertThat(newFactory().listenerFor(GitPushType.ADD),
				is(instanceOf(GitEventLogger.class)));
	}

	@Test
	public void shouldBeAbleToOverrideDefaultListener() {
		GitEventListener newDefaultListener = newListener();
		ConfigurableGitEventListenerFactory factory = newFactoryBuilder()
				.defaultListener(newDefaultListener).build();
		assertThat(factory.listenerFor(GitPushType.ADD),
				is(sameInstance(newDefaultListener)));
	}
	...
	public void shouldBeAbleToRegisterListenerForPushType() {
		...
		ConfigurableGitEventListenerFactory factory = newFactoryBuilder()
				.register(GitPushType.ADD, newListener).build();
		assertThat(factory.listenerFor(GitPushType.ADD),
				is(sameInstance(newListener)));
	}
	...
	public void shouldFindAppropriateListenerAndInvokeOnPush() {
		...
		ConfigurableGitEventListenerFactory factory = newFactoryBuilder()
				.register(GitPushType.ADD, mockListener).build();
		GitPushEvent event = GitPushEvent.aGitPushEvent()
				.pushType(GitPushType.ADD).build();
